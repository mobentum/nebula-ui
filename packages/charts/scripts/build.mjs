...
import { mkdir, readdir, readFile, rename, rm, writeFile, watch } from 'node:fs/promises';
...
if (duplicates.length) {
  console.error(
    `[nebula-charts] duplicate output paths:\n  - ${duplicates
...
if (WATCH) {
  await buildAll().catch((err) => console.error(`[nebula-charts] ${err.message}`));
  console.log(`[nebula-charts] watching ${entryPoints.length} modules…`);
  ...
  const rebuild = () => {
    ...
    timer = setTimeout(async () => {
      try {
        await buildAll();
        console.log('[nebula-charts] rebuilt');
      } catch (err) {
        console.error(`[nebula-charts] ${err.message}`);
      }
    }, 150);
  };
  ...
  try {
    await buildAll();
    console.log(`[nebula-charts] built ${entryPoints.length} modules (esm + cjs)`);
  } catch (err) {
    console.error(`[nebula-charts] ${err.message}`);
    process.exit(1);
  }
