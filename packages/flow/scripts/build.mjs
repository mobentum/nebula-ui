...
if (duplicates.length) {
  console.error(
    `[nebula-flow] duplicate output paths:\n  - ${duplicates
...
if (WATCH) {
  await buildAll().catch((err) => console.error(`[nebula-flow] ${err.message}`));
  console.log(`[nebula-flow] watching ${entryPoints.length} modules…`);
  ...
  const rebuild = () => {
    ...
    timer = setTimeout(async () => {
      try {
        await buildAll();
        console.log('[nebula-flow] rebuilt');
      } catch (err) {
        console.error(`[nebula-flow] ${err.message}`);
      }
    }, 150);
  };
  ...
  try {
    await buildAll();
    console.log(`[nebula-flow] built ${entryPoints.length} modules (esm + cjs)`);
  } catch (err) {
    console.error(`[nebula-flow] ${err.message}`);
    process.exit(1);
  }
